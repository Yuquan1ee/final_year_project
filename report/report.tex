...
\graphicspath{{images/}}
\onehalfspacing
\emergencystretch=2em                 % Allow slightly looser spacing to prevent overfull boxes
...
\hypersetup{
    ...
    urlcolor=blue,
    citecolor=black,
    linktoc=all,          % Link both section titles and page numbers in TOC/LOF/LOT
}
...
Lastly, I would like to thank my examiner, A/P Lin Guosheng, for taking the time to review and evaluate this final year project.
...
\label{sec:diffusion_inpainting}
...
Preconditions & \begin{minipage}[t]{\linewidth}\begin{enumerate}[nosep,leftmargin=*,topsep=0pt,partopsep=0pt,after=\vspace{2pt}]
    ...
    \item The user has an image file in a supported format (JPEG, PNG).
...
\end{enumerate}\end{minipage} \\ \hline
Includes & -- \\ \hline
Special Requirements & Image files must not exceed 10 MB. Supported formats: JPEG, PNG. \\ \hline
...
Flow of Events & \begin{minipage}[t]{\linewidth}\begin{enumerate}[nosep,leftmargin=*,topsep=0pt,partopsep=0pt,after=\vspace{2pt}]
    ...
    \item The user optionally selects a model (UC-7) and adjusts the strength parameter to control style intensity.
...
Flow of Events & \begin{minipage}[t]{\linewidth}\begin{enumerate}[nosep,leftmargin=*,topsep=0pt,partopsep=0pt,after=\vspace{2pt}]
    ...
    \item The user uploads an image (UC-1).
    \item The user optionally selects a restoration model (UC-7) (e.g., CodeFormer, GFPGAN, or Real-ESRGAN).
    \item The user optionally adjusts model-specific parameters (UC-8) (e.g., fidelity weight for CodeFormer, upscale factor for Real-ESRGAN).
...
\begin{enumerate}[label=\textbf{FR-\arabic*}.,start=1]
    \item The system shall accept image uploads in JPEG and PNG formats.
...
% Iterative methodology diagram created in draw.io
...
% System architecture diagram created in draw.io
...
\begin{itemize}
    ...
    \item \textbf{Tab Components} (\texttt{components/}): Each editing feature is encapsulated in its own tab component --- \texttt{InpaintingTab.tsx}, \texttt{Style\-Transfer\-Tab.tsx}, and \texttt{Restoration\-Tab.tsx}. Each tab manages its own local state for image data, generation parameters, and UI state (loading, errors, results). This design ensures that tabs are independent and do not share mutable state.
...
\begin{itemize}
    \item \textbf{Router Layer} (\texttt{routers/}): Three router modules --- \texttt{inpainting.py}, \texttt{style\_\allowbreak{}transfer.py}, and \texttt{restoration.py} --- define the API endpoints. Each router receives HTTP requests, extracts validated parameters from Pydantic schemas, delegates processing to the corresponding service, and returns a standardised \texttt{Image\-Response}. Routers are registered in \texttt{main.py} with URL prefixes: \texttt{/api/\allowbreak{}inpainting/}, \texttt{/api/\allowbreak{}style/}, and \texttt{/api/\allowbreak{}restoration/}.

    \item \textbf{Schema Layer} (\texttt{schemas/\allowbreak{}image.py}): Pydantic models define the API contracts. \texttt{Inpainting\-Request}, \texttt{Style\-Transfer\-Request}, and \texttt{Restoration\-Request} validate incoming data with type checking and range constraints (e.g., \texttt{guidance\_\allowbreak{}scale} between 1.0 and 20.0, \texttt{num\_\allowbreak{}inference\_\allowbreak{}steps} between 10 and 100). The shared \texttt{Image\-Response} model provides a consistent response format with fields for \texttt{success}, \texttt{image} (base64), \texttt{error}, \texttt{model\_used}, and \texttt{processing\_\allowbreak{}time}.

    \item \textbf{Service Layer} (\texttt{services/}): The \texttt{Diffusion\-Service} class manages diffusion model pipelines for inpainting and style transfer, while the \texttt{Restoration\-Service} handles CodeFormer, GFPGAN, and Real-ESRGAN models. Services are instantiated as \textbf{singletons} using factory functions (\texttt{get\_diffusion\_service()} and \texttt{get\_restoration\_service()}), ensuring that loaded models are cached in GPU memory across multiple requests. This avoids redundant model loading and reduces response latency for subsequent requests.
\end{itemize}
...
This section presents the implementation of DiffusionDesk, covering the backend services that perform AI-powered image processing (Section~\ref{sec:backend}) and the frontend application that provides the user interface (Section~\ref{sec:frontend}).

\subsection{Backend Development}
\label{sec:backend}

The backend is built with FastAPI, a modern Python web framework chosen for its automatic request validation via Pydantic, built-in OpenAPI documentation, and native \texttt{async} support. The backend is designed to run on GPU-equipped machines (Google Colab, cloud instances, or local workstations).

\subsubsection{Project Structure}
\label{sec:backend_structure}

The backend follows a layered architecture that separates routing, validation, and inference concerns. API requests flow through three layers: \textbf{routers} handle HTTP endpoints and delegate to \textbf{services}, which contain the ML inference logic; \textbf{schemas} define Pydantic models for request validation and response serialisation. Each service is implemented as a singleton to ensure that expensive GPU models are loaded only once and reused across requests.

The backend directory follows a flat, three-folder layout under \texttt{app/}: \texttt{routers/} contains one module per feature (\texttt{inpainting.py}, \texttt{style\_transfer.py}, \texttt{restoration.py}), \texttt{services/} contains \texttt{diffusion.py} (for inpainting and style transfer) and \texttt{restoration.py} (for face and upscaling models), and \texttt{schemas/image.py} defines Pydantic request/response models shared across all endpoints.

The application entry point (\texttt{main.py}) initialises a FastAPI instance with a \texttt{lifespan} context manager for startup/shutdown events, configures CORS middleware with \texttt{allow\_origins=["*"]} to permit cross-origin requests from the frontend (necessary when the frontend runs on a different domain, e.g., Vercel, while the backend runs on Colab via ngrok), mounts the three feature routers under \texttt{/api/} prefixes, and exposes a \texttt{/api/health} endpoint that reports GPU availability and currently loaded pipelines.

\subsubsection{API Design}
\label{sec:api_design}

The API uses JSON request bodies with base64-encoded images rather than multipart form uploads. This design simplifies the frontend integration---the React client can construct the entire request as a single \texttt{JSON.stringify()} call---and allows Pydantic to validate all fields (including numeric constraints) in one pass. Every endpoint returns a uniform \texttt{ImageResponse} containing a success flag, the result image (base64), the model used, and the processing time.

The schema layer (\texttt{schemas/image.py}) defines Pydantic models for each request type. Pydantic \texttt{Field} constraints enforce value ranges at the API boundary---for example, \texttt{guidance\_scale} is clamped between 1.0 and 20.0, \texttt{num\_inference\_steps} between 10 and 100, and \texttt{seed} between 0 and $2^{31}-1$. String enums (\texttt{InpaintingModel}, \texttt{FaceModel}, \texttt{UpscaleOption}) restrict model and option selection to known-valid keys, preventing invalid values from reaching the service layer. All endpoints return a uniform \texttt{ImageResponse} model containing a success flag, the result image (base64), the model used, and the processing time.

Table~\ref{tab:api_endpoints} summarises the complete API surface.
...
\midrule
\texttt{/api/inpainting/} & POST & Inpaint masked region with a text prompt \\
\texttt{/api/inpainting/models} & GET & List available inpainting models and VRAM requirements \\
\texttt{/api/style/} & POST & Apply style transfer with a preset or custom prompt \\
\texttt{/api/style/presets} & GET & List available style presets \\
\texttt{/api/restoration/} & POST & Run restoration pipeline (face, upscale, scratch, colour) \\
\texttt{/api/restoration/models} & GET & List available restoration models \\
\texttt{/api/health} & GET & Health check with GPU info and loaded pipelines \\
\bottomrule
\end{tabularx}
\end{table}

\subsubsection{Inpainting Service}
\label{sec:inpainting_service}

The inpainting feature supports four model families---Stable Diffusion 1.5, SDXL, Kandinsky~2.2, and FLUX.1 Fill---each available in multiple quantisation variants. Rather than hard-coding pipeline parameters, the service uses a declarative configuration dictionary (Listing~\ref{lst:inpainting_models}) where each entry specifies the Hugging Face model~ID, pipeline class name, data type, and optional quantisation or CPU-offload flags. This design makes adding new models a one-line dictionary entry rather than a code change.

\begin{lstlisting}[language=Python, caption={Declarative model configuration dictionary (excerpt from \texttt{diffusion.py})}, label={lst:inpainting_models}]
INPAINTING_MODELS = {
    "sd-inpainting": {
        "model_id": "runwayml/"
    ...
    "sdxl-inpainting": {
        "model_id": "diffusers/stable-diffusion-"
                    "xl-1.0-inpainting-0.1",
        "pipeline":
    ...
    "sdxl-inpainting-4bit": {
        ...
        "pipeline":
            "StableDiffusionXLInpaintPipeline",
        "torch_dtype": "float16",
        "quantization": "4bit",  # ~4 GB VRAM
    },
    "kandinsky-inpainting": {
        "model_id": "kandinsky-community/"
                    "kandinsky-2-2-decoder-inpaint",
        "pipeline": "AutoPipelineForInpainting",
        "torch_dtype": "float16",
    },
    "flux-fill-nf4": {
        "model_id": "black-forest-labs/"
                    "FLUX.1-Fill-dev",
        "pipeline": "FluxFillPipeline",
        "torch_dtype": "float16",
        "quantization": "nf4",  # ~10 GB VRAM
    },
    # ... additional variants omitted
}
\end{lstlisting}

The \texttt{inpaint()} method (Listing~\ref{lst:inpaint_method}) implements the full request flow. It decodes the base64 image and mask, records the original dimensions, resizes inputs to the model's optimal resolution (512$\times$512 for SD~1.5, 1024$\times$1024 for SDXL and FLUX), loads the pipeline via the caching mechanism described in Section~\ref{sec:vram_optimization}, and runs inference under \texttt{torch.inference\_mode()} to disable gradient tracking. The result is resized back to the original dimensions and returned as a base64 string. All parameters---guidance scale, inference steps, seed, strength, and padding mask crop---are forwarded from the validated request schema.

\begin{lstlisting}[language=Python, caption={Inpainting inference method (\texttt{diffusion.py})}, label={lst:inpaint_method}]
def inpaint(self, image_b64, mask_b64, prompt,
            model_key="sd-inpainting", **kwargs):
    start_time = time.time()
    try:
        config = INPAINTING_MODELS[model_key]
        image, fmt = self.base64_to_image(image_b64)
        mask = self.base64_to_mask(mask_b64)
        original_size = image.size

        # Resize to model-optimal dimensions
        if "sdxl" in model_key \
                or "flux" in model_key:
        ...
        result_b64 = self.image_to_base64(
            result, fmt)
...
Each router endpoint follows the same pattern: obtain the singleton service instance, unpack the validated request fields, delegate to the corresponding service method, and wrap the result in an \texttt{ImageResponse}. This uniform structure is repeated across all three routers (\texttt{inpainting.py}, \texttt{style\_transfer.py}, \texttt{restoration.py}).

\subsubsection{Style Transfer Service}
\label{sec:style_transfer_service}

Style transfer is implemented using diffusion-based image-to-image (img2img) generation rather than traditional neural style transfer \citep{gatys2016style}. This approach leverages the same Stable Diffusion and SDXL models used for inpainting, but in img2img mode: the input image is partially noised according to a \texttt{strength} parameter and then denoised with a style-describing prompt. Higher strength values produce more stylised output at the cost of reduced fidelity to the original image.

The system provides nine predefined style presets (Listing~\ref{lst:style_config}), each mapping a user-friendly name to an optimised prompt string. The \texttt{STYLE\_PROMPTS} dictionary serves as the preset library, while \texttt{STYLE\_MODELS} mirrors the structure of \texttt{INPAINTING\_MODELS} for img2img pipelines. Users may also supply a custom text prompt via the ``custom'' preset option.

\begin{lstlisting}[language=Python, caption={Style presets and model configuration (\texttt{diffusion.py})}, label={lst:style_config}]
STYLE_PROMPTS = {
    "oil_painting": "oil painting style, thick "
        "brushstrokes, vibrant colors, masterpiece",
    "watercolor": "watercolor painting style, "
        "soft edges, flowing colors, delicate",
    "anime": "anime style, cel shading, vibrant "
        "colors, japanese animation, detailed",
    "sketch": "pencil sketch, black and white, "
        "detailed linework, hand drawn",
    "cyberpunk": "cyberpunk style, neon lights, "
        "futuristic, dark atmosphere, sci-fi",
    "ghibli": "studio ghibli style, soft colors, "
        "whimsical, animated, magical",
    "pixel_art": "pixel art style, 8-bit, retro "
        "video game aesthetic, nostalgic",
    "pop_art": "pop art style, bold colors, comic "
        "book, andy warhol inspired",
    "impressionist": "impressionist painting, monet "
        "style, soft brushstrokes, light and color",
}
...
The \texttt{style\_transfer()} method (Listing~\ref{lst:style_transfer_method}) follows the same pattern as inpainting: decode, resize, load pipeline, build keyword arguments, run inference, resize back, and encode. The key difference is that it looks up the style prompt from \texttt{STYLE\_PROMPTS} (falling back to the raw string for custom styles) and passes \texttt{strength} as the denoising strength.

\begin{lstlisting}[language=Python, caption={Style transfer inference method (\texttt{diffusion.py})}, label={lst:style_transfer_method}]
def style_transfer(self, image_b64, style,
        model_key="sdxl-img2img", strength=0.6,
        **kwargs):
    start_time = time.time()
    try:
        image, fmt = self.base64_to_image(
            image_b64)
        original_size = image.size
        ...
        # Resolve style preset or use custom text
        style_prompt = STYLE_PROMPTS.get(
            style, style)

        config = STYLE_MODELS[model_key]
        pipe = self._load_pipeline(
            f"style-{model_key}", config)
        ...
        return self.image_to_base64(result, fmt),\
...
\label{sec:restoration_service}

The restoration feature uses specialised computer vision models rather than diffusion models. It supports four operations that can be combined in a single request: scratch removal, face enhancement (Code\-Former or GFPGAN), colourisation, and super-resolution upscaling (Real-ESRGAN). The pipeline ordering is deliberate: scratches are removed first to give face detection a cleaner input, faces are enhanced next, colourisation follows (operating on the enhanced image), and upscaling is applied last to avoid processing unnecessarily large images in earlier stages.

Listing~\ref{lst:restore_pipeline} shows the main \texttt{restore()} method that orchestrates this pipeline. Each stage is gated by a boolean flag from the request, and the intermediate result is passed through sequentially.

\begin{lstlisting}[language=Python, caption={Restoration pipeline orchestration (\texttt{restoration.py})}, label={lst:restore_pipeline}]
def restore(self, image_b64,
        enable_face_enhance=True,
        face_model="codeformer",
        fidelity=0.5, upscale="2x",
    ...
    try:
        ...
        if enable_face_enhance:
            if face_model == "codeformer":
                result = \
                    self.enhance_faces_codeformer(
                        result, fidelity)
            else:
                result = \
                    self.enhance_faces_gfpgan(result)

        # 3. Colourisation
        if enable_colorize:
            result = self.colorize(result)

        # 4. Upscaling (last to preserve quality)
        if upscale != "none":
            scale = 4 if upscale == "4x" else 2
            result = self.upscale(result, scale)
...
Listing~\ref{lst:codeformer_method} shows the CodeFormer face enhancement implementation. The method uses a \texttt{FaceRestoreHelper} utility to detect faces via RetinaFace, align and warp each face to a canonical 512$\times$512 crop, process it through the CodeFormer transformer model with the fidelity parameter \texttt{w}, and then paste the restored face back into the original image using an inverse affine transform. The \texttt{fidelity} parameter controls the trade-off between enhancement quality (\texttt{w}$=$0) and faithfulness to the original face (\texttt{w}$=$1).

\begin{lstlisting}[language=Python, caption={CodeFormer face enhancement (\texttt{restoration.py})}, label={lst:codeformer_method}]
def enhance_faces_codeformer(self, image, fidelity=0.5):
    import torch
    ...
    for idx, cropped_face in enumerate(
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Lazy loading with caching.} Models are loaded into GPU memory only on first use and cached in a \texttt{\_pipelines} dictionary. Subsequent requests for the same model reuse the cached pipeline, avoiding repeated downloads and weight loading.

    \item \textbf{Quantisation.} For SDXL and FLUX models, the system supports 8-bit and 4-bit quantisation via BitsAndBytes \citep{Dettmers2022}. This reduces VRAM usage significantly: SDXL drops from 10--12~GB to approximately 4~GB at 4-bit precision, and FLUX.1~Fill drops from 22--24~GB to approximately 10~GB at NF4 precision.

    \item \textbf{Attention slicing.} All pipelines enable attention slicing via \texttt{enable\_\-attention\_\-slicing()}, which computes attention in sequential chunks rather than all at once, trading speed for lower peak memory.

    \item \textbf{CPU offloading.} For FLUX.1~Fill at full precision, the service uses \texttt{enable\_\-model\_\-cpu\_\-offload()}, which keeps model weights on the CPU and transfers them to the GPU only during the forward pass of each component.

    \item \textbf{Inference mode.} All inference runs under \texttt{torch.inference\_mode()}, which disables gradient computation and autograd bookkeeping, reducing memory overhead.
\end{enumerate}

Listing~\ref{lst:flux_quantized} shows the FLUX.1~Fill quantisation loader. Because FLUX has two large components---the transformer and the T5 text encoder---both must be quantised separately using their respective BitsAndBytesConfig classes (from \texttt{diffusers} and \texttt{transformers}). The quantised components are then passed to \texttt{FluxFillPipeline.from\_pretrained()} with \texttt{device\_map="auto"} for automatic device placement.

\begin{lstlisting}[language=Python, caption={FLUX.1 Fill quantised loading (\texttt{diffusion.py})}, label={lst:flux_quantized}]
def _load_flux_fill_quantized(self, config,
        quantization, torch_dtype):
    from diffusers import FluxFillPipeline, AutoModel
    from diffusers import BitsAndBytesConfig \
        as DiffusersBnBConfig
    from transformers import T5EncoderModel
    from transformers import BitsAndBytesConfig \
        as TransformersBnBConfig

    model_id = config["model_id"]

    if quantization == "8bit":
        diff_qc = DiffusersBnBConfig(
            load_in_8bit=True)
        trans_qc = TransformersBnBConfig(
            load_in_8bit=True)
    else:  # 4bit or nf4
        quant_type = "nf4" \
            if quantization == "nf4" else "fp4"
        diff_qc = DiffusersBnBConfig(
            load_in_4bit=True,
    ...
    transformer = AutoModel.from_pretrained(
        ...
        quantization_config=diff_qc,
        torch_dtype=torch_dtype)

    text_encoder_2 = T5EncoderModel.from_pretrained(
        model_id, subfolder="text_encoder_2",
        quantization_config=trans_qc,
        torch_dtype=torch_dtype)
...
Listing~\ref{lst:load_pipeline} shows the general pipeline loader, which handles three cases: FLUX quantisation (delegated to the method above), SD/SDXL quantisation via \texttt{Pipeline\-Quantization\-Config} (which automatically targets the UNet component), and standard full-precision loading. For non-quantised large models, CPU offloading is enabled when the configuration flag is set.

\begin{lstlisting}[language=Python, caption={General pipeline loading with caching (\texttt{diffusion.py})}, label={lst:load_pipeline}]
def _load_pipeline(self, pipeline_key, config):
    ...
    if quantization and \
            config["pipeline"] == "FluxFillPipeline":
        pipe = self._load_flux_fill_quantized(
            config, quantization, torch_dtype)
    elif quantization:
        # SD/SDXL: PipelineQuantizationConfig
        from diffusers.quantizers import \
            PipelineQuantizationConfig
        pipeline_quant_config = \
            PipelineQuantizationConfig(
                quant_backend=
                    f"bitsandbytes_{quantization}",
                quant_kwargs={
...
Table~\ref{tab:vram_requirements} summarises the VRAM requirements for each model at different precision levels.

\begin{table}[H]
\centering
\caption{VRAM requirements by model and precision}
\label{tab:vram_requirements}
\begin{tabularx}{\textwidth}{X c c c}
\toprule
\textbf{Model} & \textbf{FP16/BF16} & \textbf{8-bit} & \textbf{4-bit} \\
\midrule
SD 1.5 Inpainting & 5--7 GB & --- & --- \\
SDXL Inpainting & 10--12 GB & $\sim$6 GB & $\sim$4 GB \\
Kandinsky 2.2 & 6--8 GB & --- & --- \\
FLUX.1 Fill & 22--24 GB & $\sim$16 GB & $\sim$10 GB \\
SDXL img2img & 10--12 GB & $\sim$6 GB & $\sim$4 GB \\
...
The frontend source code is organised under \texttt{src/} with two main directories: \texttt{api/} contains the API configuration (\texttt{config.ts}), client functions (\texttt{imageApi.ts}), and barrel exports; \texttt{components/} contains the four tab components (\texttt{HomeTab}, \texttt{InpaintingTab}, \texttt{StyleTransferTab}, \texttt{RestorationTab}) and the shared \texttt{MaskCanvas} drawing component. The root \texttt{App.tsx} manages tab navigation, and \texttt{main.tsx} serves as the React entry point.

Table~\ref{tab:frontend_deps} lists the key frontend dependencies.

\begin{table}[H]
\centering
\caption{Key frontend dependencies}
...
The application uses a tab-based navigation pattern with a shared header. The dark theme (Tailwind \texttt{slate-900} background with \texttt{indigo-600} accent) provides comfortable viewing during extended editing sessions and reduces visual distraction from the images being edited. Each feature tab follows a consistent three-step workflow: upload an image, configure parameters, and generate a result. Common UI patterns---error banners, loading spinners, before/after comparison, and download buttons---are repeated across all tabs for consistency.

The root \texttt{App.tsx} component manages tab navigation using React's \texttt{useState} hook. Tab definitions are stored as a typed array of \texttt{\{id, label\}} objects, and the active tab is rendered via conditional rendering. The dark theme uses Tailwind's \texttt{slate-900} background with \texttt{indigo-600} accents. Each tab is self-contained, managing its own local state for image data, generation parameters, and UI state. The wireframes in Section~\ref{sec:wireframes} illustrate the layout of each tab.

\subsubsection{Canvas and Mask Drawing}
\label{sec:canvas_mask}

The inpainting feature requires users to paint a mask over the regions they want to edit. The \texttt{MaskCanvas} component implements this using a dual-element architecture: an \texttt{<img>} element displays the source image, and a \texttt{<canvas>} element is positioned absolutely on top as a transparent overlay. This separation ensures that the mask drawing does not modify the original image data.

A key challenge is coordinate scaling: the canvas has pixel dimensions matching the original image (e.g., 1920$\times$1080), but is displayed at the container's CSS width (e.g., 800 pixels wide). The \texttt{canvasScale} factor (container width $/$ image width) maps mouse/touch coordinates from display space to canvas space. This scale is recalculated on window resize.

Listing~\ref{lst:draw_fn} shows the \texttt{draw()} function. The brush tool uses \texttt{source-over} compositing to paint semi-transparent red strokes; the eraser uses \texttt{destination-out} compositing, which removes pixels from the canvas (effectively erasing previously painted mask regions). Lines are drawn between consecutive positions to produce smooth strokes.

\begin{lstlisting}[language={[LaTeX]TeX}, caption={Canvas drawing function (\texttt{MaskCanvas.tsx})}, label={lst:draw_fn}, morekeywords={const,if,else,return,function,useCallback}]
const draw = useCallback((x: number, y: number) => {
  ...
  if (lastPosRef.current) {
    ctx.moveTo(lastPosRef.current.x,
      lastPosRef.current.y)
    ctx.lineTo(x, y)
    ctx.stroke()
  } else {
    ctx.arc(x, y, brushSize / 2,
      0, Math.PI * 2)
    ctx.fill()
  }
...
When the user finishes drawing, the mask must be exported as a black-and-white PNG for the backend (white pixels indicate regions to inpaint). Listing~\ref{lst:export_mask} shows the \texttt{exportMask()} function, which creates a temporary canvas, fills it with black, reads the alpha channel of the drawing canvas, and sets any pixel with alpha $> 0$ to white. The result is exported as a data URL.

\begin{lstlisting}[language={[LaTeX]TeX}, caption={Mask export function (\texttt{MaskCanvas.tsx})}, label={lst:export_mask}, morekeywords={const,if,return,for,let,function,useCallback,null}]
const exportMask = useCallback(() => {
  ...
  // Alpha > 0 becomes white (inpaint region)
  for (let i = 0;
...
The frontend communicates with the backend through a thin API client layer in \texttt{src/api/}. The configuration module (\texttt{config.ts}) reads the backend URL from the \texttt{VITE\_API\_URL} environment variable set at build time by Vite, defaulting to \texttt{http://localhost:8000}. This allows the same codebase to target a local development server or a remote Colab instance by changing a single \texttt{.env} file. The module also provides utility functions for converting between \texttt{File} objects and base64 strings using the \texttt{FileReader} API.

Listing~\ref{lst:inpaint_api} shows the \texttt{inpaintImage()} API function as a representative example of the API client pattern. It converts the uploaded \texttt{File} to base64, constructs a JSON body with snake\_case keys (matching the Pydantic schema), sends a POST request, and converts the response image from base64 to a data URL for display in an \texttt{<img>} tag.

\begin{lstlisting}[language={[LaTeX]TeX}, caption={Inpainting API client function (\texttt{imageApi.ts})}, label={lst:inpaint_api}, morekeywords={export,async,function,const,await,if,return,throw,new}]
export async function inpaintImage(
    params: InpaintingParams
  ): Promise<ImageResponse> {
  const imageBase64 =
    await fileToBase64(params.image);

  const response = await fetch(
    `${API_BASE_URL}/api/inpainting/`, {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      ...
      padding_mask_crop:
        params.paddingMaskCrop ?? null,
    }),
  });

  if (!response.ok)
    throw new Error(
      `Inpainting failed: ${response.statusText}`
    );
...
TypeScript interfaces (\texttt{InpaintingParams}, \texttt{StyleTransferParams}, \texttt{RestorationParams}, \texttt{ImageResponse}) mirror the backend Pydantic schemas, providing compile-time type safety for API calls. The frontend uses camelCase naming conventions (e.g., \texttt{negativePrompt}), which are mapped to the backend's snake\_case keys (e.g., \texttt{negative\_prompt}) in the \texttt{JSON.stringify()} call. The style transfer and restoration API functions follow the same pattern as \texttt{inpaintImage()}.
...
This section presents the results of DiffusionDesk's three core features---inpainting, style transfer, and image restoration---evaluated using a curated set of test images. All inference was performed on a Google Colab environment with an NVIDIA T4 GPU (15\,GB VRAM). The test images, prompts, and configurations are documented in the project's test plan (\texttt{test/TEST\_PLAN.md}).

\subsection{Inpainting Results}

Inpainting was tested across four scenarios using all four supported models: Stable Diffusion Inpainting (SD~1.5), SDXL Inpainting, Kandinsky 2.2 Inpainting, and FLUX.1 Fill (NF4). Each test involved masking a region of the source image and providing a text prompt describing the desired replacement content. Default parameters were used: guidance scale 7.5, 30 inference steps, and strength 1.0.

\subsubsection{Object Removal: Beach Scene}

Figure~\ref{fig:inpaint_beach} demonstrates large-area object removal on a crowded beach photograph. A group of beachgoers was masked and the prompt ``empty sandy beach'' was used with SD Inpainting. The model successfully removed the people and generated a plausible continuation of the sand and shoreline. The colour tone and lighting of the generated region blend well with the surrounding unmasked areas, though some minor texture artefacts are visible on the sand surface where the model filled the large masked region.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_beach_original.jpg}
        \subcaption{Original}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_beach_mask.png}
        \subcaption{Mask overlay}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_beach_result.png}
        \subcaption{Result}
    \end{minipage}
    \caption{Inpainting result: removing beachgoers from a crowded beach scene (SD Inpainting, strength 1.0).}
    \label{fig:inpaint_beach}
\end{figure}

\subsubsection{Object Removal: Vehicle}

Figure~\ref{fig:inpaint_car} shows the removal of a large foreground object---a vintage car occupying the majority of the frame. Using SDXL Inpainting with the prompt ``empty cobblestone road,'' the model replaced the car with a detailed cobblestone road surface and extended the row of building fa\c{c}ades with varied colours and architectural details. The generated perspective and texture are consistent with the surrounding scene, demonstrating SDXL's ability to synthesise complex content for large masked regions.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_car_original.jpg}
        \subcaption{Original}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_car_result.png}
        \subcaption{Result (SDXL Inpainting)}
    \end{minipage}
    \caption{Inpainting result: removing a vintage car and generating an empty cobblestone road.}
    \label{fig:inpaint_car}
\end{figure}

\subsubsection{Scene Modification: Mountain Landscape}

Figure~\ref{fig:inpaint_mountain} illustrates scene modification on a mountain landscape. A mountain peak was masked and the prompt ``clear blue sky with clouds'' was used with FLUX.1 Fill (NF4 quantisation). The model replaced the peak with a realistic sky that matches the surrounding atmosphere and horizon line. FLUX.1 Fill produced particularly natural transitions at the mask boundary, with no visible seams or colour discontinuities.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_mountain_original.jpg}
        \subcaption{Original}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_mountain_mask.png}
        \subcaption{Mask overlay}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_mountain_result.png}
        \subcaption{Result}
    \end{minipage}
    \caption{Inpainting result: replacing a mountain peak with sky (FLUX.1 Fill NF4, strength 1.0).}
    \label{fig:inpaint_mountain}
\end{figure}

\subsubsection{Scene Modification: Coastal Landscape}

Figure~\ref{fig:inpaint_coastal} demonstrates inpainting on a coastal scene using Kandinsky 2.2 Inpainting. The foreground grasses were masked with the prompt ``calm ocean water by the beach.'' While the model correctly generates ocean water in the masked region, the lighting and colour tone of the generated content does not blend seamlessly with the surrounding unmasked area---the inpainted water appears noticeably different from the existing ocean in the background. This highlights a limitation of Kandinsky 2.2's image-prior architecture (based on CLIP image embeddings): although it understands the semantic content to generate, it does not condition as strongly on the surrounding pixel context as the Stable Diffusion family, leading to visible colour discontinuities at mask boundaries.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_coastal_original.jpg}
        \subcaption{Original}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/inpaint_coastal_result.png}
        \subcaption{Result (Kandinsky 2.2 Inpainting)}
    \end{minipage}
    \caption{Inpainting result: replacing foreground grasses with ocean water (Kandinsky 2.2).}
    \label{fig:inpaint_coastal}
\end{figure}

\subsubsection{Inpainting Discussion}

Across all test cases, the inpainting results demonstrate strong prompt adherence and natural boundary blending. The key observations are:

\begin{itemize}
    \item \textbf{SD Inpainting} performs well for moderate-sized masks and produces results with good colour consistency, though fine details can appear slightly soft.
    \item \textbf{SDXL Inpainting} generates sharper, higher-resolution details and handles large masked regions more convincingly, at the cost of higher VRAM usage (approximately 6\,GB with 4-bit quantisation vs 5\,GB for SD~1.5).
    \item \textbf{Kandinsky 2.2 Inpainting} generates semantically correct content but can exhibit colour and lighting mismatches at mask boundaries. Its CLIP image-prior architecture prioritises semantic understanding over pixel-level coherence with the surrounding region, making it less suitable for tasks requiring seamless blending.
    \item \textbf{FLUX.1 Fill (NF4)} produces the most natural mask-boundary transitions and demonstrates superior understanding of spatial context, but requires approximately 10\,GB VRAM even with NF4 quantisation.
    \item Mask precision directly impacts output quality---cleanly drawn masks that follow object contours yield more natural results than rough selections.
\end{itemize}

\subsection{Style Transfer Results}

Style transfer was evaluated using SDXL img2img with five built-in style presets. A strength of 0.6 (moderate stylisation) was used for all tests, with guidance scale 7.5 and 30 inference steps.

\subsubsection{Oil Painting Style}

Figure~\ref{fig:style_oil} shows a waterfall landscape transformed into an oil painting. The model successfully applied visible brushstroke textures, rich colour saturation, and the impasto-like quality characteristic of oil paintings. The overall composition and spatial structure of the original photograph are preserved, while the rendering style is convincingly painterly.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_landscape_original.jpg}
        \subcaption{Original photograph}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_landscape_oil.jpg}
        \subcaption{Oil painting style}
    \end{minipage}
    \caption{Style transfer: waterfall landscape transformed to oil painting (SDXL img2img, strength 0.6).}
    \label{fig:style_oil}
\end{figure}

\subsubsection{Multi-Style Comparison: Architecture}

Figure~\ref{fig:style_multi} demonstrates the diversity of stylistic output by applying two different presets to the same European canal photograph. The impressionist style produces soft, diffused colours and loose brushwork reminiscent of Monet, while the sketch style converts the scene to detailed pencil linework with selective colour accents. Both outputs maintain the original composition and architectural structure.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_architecture_original.jpg}
        \subcaption{Original}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_architecture_impressionist.jpg}
        \subcaption{Impressionist}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_architecture_sketch.jpg}
        \subcaption{Sketch}
    \end{minipage}
    \caption{Style transfer: European canal scene with two different style presets (SDXL img2img, strength 0.6).}
    \label{fig:style_multi}
\end{figure}

\subsubsection{Anime Style: Portrait}

Figure~\ref{fig:style_anime} shows a portrait photograph converted to anime style. The model transforms the subject's features into the characteristic flat-shaded, large-eyed aesthetic of Japanese animation while preserving the pose, accessories (sunglasses), and clothing pattern. The background is simplified into soft gradients consistent with anime art conventions.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_portrait_original.jpg}
        \subcaption{Original photograph}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/style_portrait_anime.jpg}
        \subcaption{Anime style}
    \end{minipage}
    \caption{Style transfer: portrait converted to anime style (SDXL img2img, strength 0.6).}
    \label{fig:style_anime}
\end{figure}

\subsubsection{Style Transfer Discussion}

The style transfer results show that prompt-guided img2img with SDXL produces high-quality stylistic transformations across diverse styles and subject matter. Key observations:

\begin{itemize}
    \item The \textbf{strength parameter} is the primary quality control. At 0.6, the model achieves a good balance---strong enough to transform the rendering style while preserving the original composition. Lower values (0.2--0.4) produce subtle filter-like effects; higher values (0.8--1.0) can significantly alter the image structure.
    \item \textbf{Style consistency} varies by preset. Structured styles (oil painting, sketch) produce more consistent results across different images, while abstract styles (cyberpunk, pixel art) can be more unpredictable.
    \item \textbf{Portrait subjects} are handled well, with facial features adapted to match the target art style without losing the subject's identity at moderate strength values.
\end{itemize}

\subsection{Image Restoration Results}

Image restoration was evaluated using five degraded face photographs from the GFPGAN test dataset. Tests covered both CodeFormer (fidelity 0.5) and GFPGAN, with Real-ESRGAN integrated as the background upsampler.

\subsubsection{Face Enhancement: CodeFormer}

Figure~\ref{fig:restore_face01} shows a low-quality cropped child face restored using CodeFormer with 2x upscaling. The model sharpens facial features---eyes, teeth, and skin texture are visibly improved---while reducing compression artefacts and noise. The overall clarity is noticeably enhanced compared to the degraded input.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restore_face01_original.png}
        \subcaption{Degraded input}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restored_face_01.png}
        \subcaption{Restored (CodeFormer, fidelity 0.5, 2x)}
    \end{minipage}
    \caption{Face restoration: cropped child face enhanced with CodeFormer and Real-ESRGAN 2x.}
    \label{fig:restore_face01}
\end{figure}

\subsubsection{Multi-Face Enhancement: Full Scene}

Figure~\ref{fig:restore_face02} shows CodeFormer applied to a full-scene photograph containing two children. The model detects and enhances both faces independently while Real-ESRGAN upscales the background. The facial details of both subjects---hair clips, clothing texture, and facial expressions---are preserved and sharpened without introducing noticeable artefacts in the non-face regions.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.38\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restore_face02_original.jpg}
        \subcaption{Degraded input}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.38\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restored_face_02.png}
        \subcaption{Restored (CodeFormer, fidelity 0.5, 2x)}
    \end{minipage}
    \caption{Face restoration: full-scene photograph with two faces enhanced using CodeFormer.}
    \label{fig:restore_face02}
\end{figure}

\subsubsection{Face Enhancement: GFPGAN}

Figure~\ref{fig:restore_face03} compares GFPGAN restoration on a grayscale child face. GFPGAN sharpens the facial features and reduces noise effectively, producing clearer eyes and smoother skin. However, as GFPGAN does not include a colourisation component, the output remains in grayscale. The enhancement is subtle but consistent, with the model preserving the subject's identity well.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restore_face03_original.png}
        \subcaption{Degraded input (B\&W)}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restored_face_03.png}
        \subcaption{Restored (GFPGAN, 2x)}
    \end{minipage}
    \caption{Face restoration: grayscale child face enhanced with GFPGAN and Real-ESRGAN 2x.}
    \label{fig:restore_face03}
\end{figure}

\subsubsection{High-Resolution Upscaling: CodeFormer with 4x}

Figure~\ref{fig:restore_face04} demonstrates CodeFormer with 4x upscaling on a low-quality cropped toddler face. The higher upscale factor quadruples the image dimensions, producing a noticeably sharper result. The model removes visible grain and noise from the original while enhancing facial details---eyes, teeth, and hair texture become significantly clearer. The 4x upscale produces a higher-resolution output than the 2x tests, making it particularly suitable for heavily degraded small photographs where both quality enhancement and resolution increase are needed.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restore_face04_original.png}
        \subcaption{Degraded input (low-quality toddler)}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restored_face_04.png}
        \subcaption{Restored (CodeFormer, 4x)}
    \end{minipage}
    \caption{Face restoration: low-quality toddler face enhanced with CodeFormer and Real-ESRGAN 4x.}
    \label{fig:restore_face04}
\end{figure}

\subsubsection{Multi-Face Enhancement: GFPGAN on Composite Image}

Figure~\ref{fig:restore_face05} shows GFPGAN applied to a side-by-side composite image containing a child photograph and an adult photograph. The model successfully detects and enhances both faces despite the different image qualities and scales. The child's face is sharpened with improved detail, while the adult's face---which was already higher quality---receives a more subtle enhancement that smooths minor imperfections without over-processing.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restore_face05_original.jpg}
        \subcaption{Degraded input (composite)}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restored_face_05.png}
        \subcaption{Restored (GFPGAN, 2x)}
    \end{minipage}
    \caption{Face restoration: side-by-side child/adult composite enhanced with GFPGAN.}
    \label{fig:restore_face05}
\end{figure}

\subsubsection{Restoration Discussion}

The restoration pipeline produces visible improvements in facial clarity and image resolution across all five test cases. Key observations:

\begin{itemize}
    \item \textbf{CodeFormer} at fidelity 0.5 provides a good balance between enhancement and identity preservation. It excels at sharpening eyes, teeth, and skin texture while maintaining natural-looking results. It also handles multi-face scenes well, enhancing each detected face independently.
    \item \textbf{GFPGAN} produces consistent, natural-looking enhancements and is particularly effective at smoothing noise and compression artefacts. It handles faces of varying quality within the same image gracefully, applying stronger enhancement to more degraded faces.
    \item \textbf{Integrated upscaling} via Real-ESRGAN as the background upsampler (the official CodeFormer/GFPGAN pipeline) produces cohesive results where both face and non-face regions are enhanced together, avoiding resolution mismatches.
    \item \textbf{Limitations}: neither model colourises black-and-white photographs---grayscale inputs remain grayscale after restoration. Face detection is a prerequisite; images where faces are too small or heavily occluded may not benefit from face-specific enhancement.
\end{itemize}

\subsection{Performance Benchmarks}

Table~\ref{tab:performance} summarises the inference times and VRAM usage measured on a Google Colab T4 GPU (15\,GB VRAM) for each model. Times exclude model loading (first-run only) and represent steady-state inference.

\begin{table}[htbp]
\centering
\caption{Inference performance on NVIDIA T4 GPU (15\,GB VRAM).}
\label{tab:performance}
\begin{tabular}{llcc}
\hline
\textbf{Task} & \textbf{Model} & \textbf{Time (s)} & \textbf{VRAM (GB)} \\
\hline
Inpainting & SD 1.5 Inpainting & 8--12 & $\sim$5 \\
Inpainting & SDXL Inpainting (4-bit) & 15--25 & $\sim$6 \\
Inpainting & Kandinsky 2.2 Inpainting & 10--15 & $\sim$7 \\
Inpainting & FLUX.1 Fill (NF4) & 30--50 & $\sim$10 \\
Style Transfer & SDXL img2img (4-bit) & 12--20 & $\sim$6 \\
Restoration & CodeFormer + ESRGAN 2x & 5--10 & $\sim$4 \\
Restoration & GFPGAN + ESRGAN 2x & 3--8 & $\sim$4 \\
\hline
\end{tabular}
\end{table}

The diffusion-based models (inpainting and style transfer) dominate both inference time and VRAM usage, with the trade-off being that higher-quality models (SDXL, FLUX.1) require proportionally more resources. The restoration models are comparatively lightweight, making them practical even on consumer hardware. Quantisation (4-bit/NF4) is essential for running SDXL and FLUX.1 models within the 15\,GB VRAM limit of Colab's T4 GPU.

\newpage

% ============================================================
% 8. PROJECT DIFFICULTIES AND LEARNING OUTCOMES
% ============================================================
...
\subsubsection{GPU Memory Constraints}
...
\subsubsection{Canvas Coordinate Scaling}

The mask drawing feature required careful handling of coordinate systems. The HTML \texttt{<canvas>} element maintains pixel dimensions matching the original uploaded image (e.g., $1920 \times 1080$), but is displayed at a smaller CSS size to fit the browser viewport. Mouse and touch coordinates are reported in CSS pixels, not canvas pixels, creating a mismatch that causes the drawn mask to appear offset from the cursor.

Solving this required computing a \texttt{canvasScale} factor (the ratio of the canvas's internal width to its displayed CSS width) and multiplying all pointer coordinates by this factor before drawing. The scale must be recalculated whenever the window is resized, which was handled using a \texttt{ResizeObserver} callback.

\subsubsection{Base64 Image Encoding}
...
This approach simplifies the API contract and makes requests self-contained, but increases payload size by approximately 33\% due to base64 encoding overhead. For the image sizes processed by this application (typically under 5\,MB), this trade-off was acceptable. However, the frontend required utility functions to convert between \texttt{File} objects, base64 strings, and data URLs, adding implementation complexity.

\subsubsection{Cross-Origin Deployment}

When deploying the backend on Google Colab with ngrok tunnelling, the frontend (hosted on \texttt{localhost} or Vercel) and backend (served through an ngrok URL) run on different origins. This triggers browser CORS (Cross-Origin Resource Sharing) restrictions, blocking API requests by default.

Configuring FastAPI's \texttt{CORSMiddleware} with \texttt{allow\_origins=["*"]} resolved this for development, but required understanding the interaction between CORS preflight requests, the ngrok proxy layer, and the browser's same-origin policy. Debugging CORS errors was time-consuming, as the browser's error messages are often generic and do not clearly indicate which header or configuration is missing.
...
\subsubsection{Diffusion Model Inference}

Working directly with diffusion model pipelines through the Hugging Face Diffusers library provided practical understanding of how these models operate at inference time. Key concepts learned include the denoising process, the role of schedulers (e.g., DDIM, Euler) in controlling the number of inference steps, the effect of guidance scale on prompt adherence, and how conditioning inputs (text prompts, mask images) guide the generation process. Understanding the distinction between latent-space diffusion (Stable Diffusion, SDXL) and pixel-space models, as well as the architectural differences between UNet-based and Transformer-based (DiT) diffusion models, deepened theoretical knowledge of the field.

\subsubsection{Model Quantisation and Memory Optimisation}
...
\subsubsection{Cloud GPU Deployment}
...
\subsection{User Authentication and History}
...
\subsection{Additional Model Support}
...
\begin{itemize}
    \item \textbf{SDXL Turbo / SD Turbo} -- Distilled models that generate results in 1--4 inference steps, enabling near-real-time editing.
    \item \textbf{Stable Diffusion 3} -- The latest generation of Stable Diffusion with improved text rendering and image quality.
    \item \textbf{IP-Adapter} -- Image prompt adapters that allow using a reference image (rather than text) to guide style transfer, enabling more intuitive style selection.
\end{itemize}

\subsection{Persistent Deployment}
...
This project set out to develop a web-based image editing application powered by diffusion models, providing accessible AI-driven tools for inpainting, style transfer, and image restoration. The resulting system, DiffusionDesk, successfully achieves the six project objectives outlined in Section~\ref{sec:introduction}.

The application integrates multiple state-of-the-art diffusion models---Stable Diffusion 1.5, SDXL, Kandinsky 2.2, and FLUX.1 Fill---for inpainting, and SDXL img2img for style transfer, within a unified FastAPI backend. Image restoration is handled by dedicated models (CodeFormer, GFPGAN, and Real-ESRGAN) integrated following the official inference pipelines. The React-based frontend provides an intuitive interface with interactive mask drawing, real-time parameter adjustment, and before/after comparison views.

Testing demonstrated that the system produces high-quality results across all three tasks. Inpainting effectively removes objects and fills masked regions with contextually appropriate content. Style transfer convincingly transforms photographs into diverse artistic styles while preserving compositional structure. Face restoration enhances degraded photographs with visible improvements in facial clarity and detail. Performance benchmarking confirmed that all models run within the 15\,GB VRAM limit of Google Colab's T4 GPU through quantisation techniques (4-bit NF4 and 8-bit), making the system accessible without specialised hardware.

Key technical contributions include the modular model-loading architecture with lazy initialisation and caching, the integration of BitsAndBytes quantisation to enable large models on consumer GPUs, and the proper implementation of the CodeFormer/GFPGAN pipeline with Real-ESRGAN as an integrated background upsampler for high-quality face restoration.

The project also identified areas for future work, including text-to-image generation, ControlNet integration for finer spatial control, batch processing, black-and-white photo colourisation, and persistent cloud deployment. The modular architecture of both the frontend and backend is designed to facilitate these extensions.
