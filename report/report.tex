...
\begin{lstlisting}[language={}, caption={Backend directory structure}, label={lst:backend_tree}]
backend/
  app/
    main.py                  # FastAPI app, CORS, router mounting
    routers/
      inpainting.py          # POST /api/inpainting/
      style_transfer.py      # POST /api/style/
      restoration.py         # POST /api/restoration/
    services/
      diffusion.py           # DiffusionService (inpainting + style)
      restoration.py         # RestorationService (faces + upscale)
    schemas/
      image.py               # Pydantic request/response models
  requirements.txt
\end{lstlisting}

Listing~\ref{lst:main_py} shows the application entry point. The \texttt{lifespan} context manager handles startup and shutdown events. CORS middleware is configured to allow all origins, which is necessary when the frontend is served from a different domain (e.g., Vercel) while the backend runs on a Colab instance exposed via ngrok. The three feature routers are mounted under \texttt{/api/} prefixes, and a health-check endpoint reports GPU availability and loaded pipelines.

\begin{lstlisting}[language=Python, caption={Application entry point (\texttt{main.py})}, label={lst:main_py}]
...
app = FastAPI(
    ...
    description="API for intelligent image editing "
...
app.include_router(inpainting.router,
    prefix="/api/inpainting", tags=["Inpainting"])
app.include_router(style_transfer.router,
    prefix="/api/style", tags=["Style Transfer"])
app.include_router(restoration.router,
    prefix="/api/restoration", tags=["Restoration"])
...
Listing~\ref{lst:schemas_response} shows the response model and the inpainting request schema. Pydantic \texttt{Field} constraints enforce value ranges at the API boundary: \texttt{guidance\_scale} is clamped between 1.0 and 20.0, \texttt{num\_inference\_steps} between 10 and 100, and \texttt{seed} between 0 and $2^{31}-1$. The \texttt{InpaintingModel} enum restricts model selection to known-valid keys, preventing typos from reaching the service layer.

\begin{lstlisting}[language=Python, caption={Response model and inpainting request schema (\texttt{schemas/image.py})}, label={lst:schemas_response}]
...
class InpaintingRequest(BaseModel):
    ...
    mask: str = Field(...,
        description="Base64 mask (white=inpaint)")
    prompt: str = Field(...,
    ...
    seed: Optional[int] = Field(
        default=None, ge=0, le=2147483647)
    strength: float = Field(
        default=1.0, ge=0.0, le=1.0)
\end{lstlisting}

Listing~\ref{lst:schemas_restoration} shows the restoration request schema and its supporting enums. The \texttt{FaceModel} enum selects between CodeFormer and GFPGAN, while \texttt{UpscaleOption} controls the Real-ESRGAN scale factor. Boolean flags enable or disable individual pipeline stages.

\begin{lstlisting}[language=Python, caption={Restoration request schema and enums (\texttt{schemas/image.py})}, label={lst:schemas_restoration}]
...
Listing~\ref{lst:inpainting_router} shows the router endpoint that connects the HTTP layer to the service. The router obtains the singleton \texttt{DiffusionService}, unpacks the validated request fields, and wraps the result in a uniform \texttt{ImageResponse}.

\begin{lstlisting}[language=Python, caption={Inpainting router endpoint (\texttt{routers/inpainting.py})}, label={lst:inpainting_router}]
from fastapi import APIRouter
from app.schemas.image import (
    InpaintingRequest, ImageResponse)
from app.services.diffusion import (
...
async def inpaint_image(
    ...
    service = get_diffusion_service()
    result_b64, error, processing_time = \
        service.inpaint(
...
\begin{lstlisting}[language={}, caption={Frontend directory structure}, label={lst:frontend_tree}]
frontend/
  src/
    api/
      config.ts            # API URL, base64 utils
      imageApi.ts          # API client functions
      index.ts             # Barrel exports
    components/
      HomeTab.tsx           # Landing page
      InpaintingTab.tsx     # Inpainting workflow
      StyleTransferTab.tsx  # Style transfer workflow
      RestorationTab.tsx    # Restoration workflow
      MaskCanvas.tsx        # Canvas mask drawing
...
Listing~\ref{lst:app_tsx} shows the complete \texttt{App.tsx} component. The tab configuration is stored as a typed array, making it straightforward to add new tabs. Active tab state is managed with React's \texttt{useState} hook, and conditional rendering displays the appropriate feature component. The wireframes in Section~\ref{sec:wireframes} illustrate the layout of each tab.

\begin{lstlisting}[language={[LaTeX]TeX}, caption={Main application shell (\texttt{App.tsx})}, label={lst:app_tsx}, morekeywords={import,from,export,default,function,const,useState,return,type}]
import { useState } from 'react'
import HomeTab from './components/HomeTab'
import InpaintingTab
  from './components/InpaintingTab'
import StyleTransferTab
  from './components/StyleTransferTab'
import RestorationTab
  from './components/RestorationTab'

type Tab = 'home' | 'inpainting'
  | 'style-transfer' | 'restoration'
...
The frontend communicates with the backend through a thin API client layer in \texttt{src/api/}. Listing~\ref{lst:config_ts} shows the configuration module, which reads the backend URL from an environment variable (\texttt{VITE\_API\_URL}) set at build time by Vite. This allows the same codebase to target a local development server or a remote Colab instance by changing a single \texttt{.env} file. The module also provides utility functions for converting between \texttt{File} objects and base64 strings.

\begin{lstlisting}[language={[LaTeX]TeX}, caption={API configuration (\texttt{config.ts})}, label={lst:config_ts}, morekeywords={export,const,import,async,function,return,new,Promise,resolve,reject,if,as}]
...
export function base64ToDataUrl(
    base64: string,
    mimeType: string = 'image/png'): string {
...
Listing~\ref{lst:inpaint_api} shows the \texttt{inpaintImage()} API function. It converts the uploaded \texttt{File} to base64, constructs a JSON body with snake\_case keys (matching the Pydantic schema), sends a POST request, and converts the response image from base64 to a data URL for display in an \texttt{<img>} tag.
...
Listing~\ref{lst:ts_interfaces} shows the TypeScript interfaces that mirror the backend Pydantic schemas. The frontend uses camelCase naming conventions (e.g., \texttt{negativePrompt}), which are mapped to the backend's snake\_case keys (e.g., \texttt{negative\_prompt}) in the \texttt{JSON.stringify()} call.

\begin{lstlisting}[language={[LaTeX]TeX}, caption={TypeScript API interfaces (\texttt{imageApi.ts})}, label={lst:ts_interfaces}, morekeywords={export,interface,string,number,boolean,null}]
