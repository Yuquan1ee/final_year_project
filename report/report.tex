...
Figure~\ref{fig:inpaint_coastal} demonstrates inpainting on a coastal scene using Kandinsky 2.2 Inpainting. The foreground grasses were masked with the prompt ``calm ocean water by the beach.'' Kandinsky 2.2 uses a distinct architecture based on image priors (CLIP image embeddings) rather than text-only conditioning, which gives it a different generation characteristic compared to the Stable Diffusion family.
