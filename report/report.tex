...
Figure~\ref{fig:inpaint_car} shows the removal of a large foreground object---a vintage car occupying the majority of the frame. Using SDXL Inpainting with the prompt ``empty cobblestone road,'' the model replaced the car with a coherent road surface and building fa\c{c}ade. The generated cobblestone texture and perspective are consistent with the surrounding scene, demonstrating SDXL's ability to handle large masked regions that require significant content generation.
...
Image restoration was evaluated using degraded face photographs from the GFPGAN test dataset. The primary model tested was CodeFormer with fidelity 0.5 (balanced mode) and Real-ESRGAN 2x upscaling integrated as the background upsampler.

\subsubsection{Face Enhancement: CodeFormer}

Figure~\ref{fig:restore_face} shows a low-quality, compressed child face photograph restored using CodeFormer. The model sharpens facial features, smooths compression artefacts, reduces noise, and produces a cleaner, more detailed output. Skin texture, eye detail, and hair strands are all visibly improved. The Real-ESRGAN background upsampler simultaneously enhances the non-face regions, producing a cohesive high-quality result.
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/restore_face01_result.png}
        \subcaption{Restored (CodeFormer, fidelity 0.5, 2x)}
    \end{minipage}
    \caption{Face restoration: CodeFormer with Real-ESRGAN 2x background upsampling.}
    \label{fig:restore_face}
\end{figure}

\subsubsection{Restoration Discussion}

The restoration pipeline produces noticeable improvements in facial clarity and image resolution. Key observations:

\begin{itemize}
    \item \textbf{CodeFormer} at fidelity 0.5 provides a good balance between enhancement and identity preservation. Lower fidelity values (0.0--0.3) produce sharper but potentially identity-altered results; higher values (0.7--1.0) make minimal changes.
    \item \textbf{Integrated upscaling} via Real-ESRGAN as the background upsampler (the official CodeFormer pipeline) produces significantly better results than applying upscaling as a separate post-processing step, as the enhanced faces are pasted onto the upscaled background at the correct resolution.
    \item \textbf{Face detection} is a prerequisite for enhancement---images where faces are too small, heavily occluded, or at extreme angles may not benefit from face-specific restoration.
    \item \textbf{Limitations}: the restoration models are trained primarily on photographic faces and may produce unnatural results on artistic or heavily stylised portraits. Colourisation for black-and-white photographs is not yet implemented.
\end{itemize}
