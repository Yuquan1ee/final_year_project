...
%
% Relationships:
%   User --> UC-1, UC-2, UC-3, UC-4, UC-5, UC-6, UC-7, UC-8, UC-9
%   System --> UC-10
...
The User actor interacts directly with all use cases except UC-10 (Load Diffusion Model), which is triggered by the System when a model is needed for inference. Key relationships include:
...
\begin{itemize}
    ...
    \item \textbf{Extend relationships}: UC-7 (Select Model) and UC-8 (Configure Generation Parameters) extend the editing use cases, as users may optionally change the model or adjust parameters. UC-5 (Select Style Preset) extends UC-4 (Apply Style Transfer).
\end{itemize}
