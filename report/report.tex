...
Figure~\ref{fig:inpaint_coastal} demonstrates inpainting on a coastal cliffside scene using Kandinsky 2.2 Inpainting. The cliff and rock formations were masked with the prompt ``calm ocean water.'' Kandinsky 2.2 uses a distinct architecture based on image priors (CLIP image embeddings) rather than text-only conditioning, which gives it a different generation characteristic compared to the Stable Diffusion family.
...
\begin{figure}[htbp]
    ...
    \end{minipage}
    \caption{Inpainting result: replacing coastal cliffs with calm ocean water (Kandinsky 2.2).}
...
\begin{itemize}
    ...
    \item \textbf{Kandinsky 2.2 Inpainting} produces distinct results owing to its CLIP image prior architecture. It handles colour blending well but can sometimes generate less structurally precise details compared to SD and SDXL.
